...
for (j in c(1:length(n_list))) {
  ...
  mXAY_1_truth <- as.numeric(
    (mA_truth + mXA_1_truth) * psi_truth + (mX_2_truth * term1_truth + term2_truth * par1_truth) * cov_truth
  )
  
  moments_truth <-  c(mA_truth,
                      mX_2_truth,
                      mXA_1_truth,
                      mXA_2_truth,
                      mAY_truth,
  ...
  ## sim_res----
  sim_res <-  foreach(i = 1:N.replicates,
                      .packages = c("glmnet", "nleqslv", "SMUT")) %dorng% {
                        result <- tryCatch({
                          if (sample_Rad & omega_11 == 1) {
                            X_1 <- matrix(2 * rbinom(n * p, size = 1, prob = 0.5) - 1,
                                          nrow = n,
                                          ncol = p)
  ...
  ## save ----
  N.success <- sum(sapply(c(1:100), function(i)
    (sim_res[[i]]$success)))
