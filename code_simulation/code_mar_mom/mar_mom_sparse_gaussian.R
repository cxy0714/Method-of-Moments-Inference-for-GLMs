...
for (j in c(1:length(n_list)) ) {
  ...
  par_truth <- c(par1_truth,par2_truth,cov_truth,psi_truth)
  ...
  moments_truth <-  c(mA_truth, mX_2_truth, mXA_1_truth, mXA_2_truth, mAY_truth, mXAY_1_truth)
  fn <- function (x) {
  ...
  sim_res <-  foreach(
    ...
    moments_em <- c(mA_em, mX_2_em, mXA_1_em, mXA_2_em, mAY_em, mXAY_1_em)
    fn_em <- function (x) {
      term0 <- forward_mapping_0(x[1],x[2])
      term1 <- forward_mapping_1(x[1], x[2])
      term2 <- forward_mapping_2(x[1], x[2])
    ...
    epsilon_par <- rnorm(4,0,1/sqrt(p))
  ...
  ## save ----
  sols_em <-  matrix(NA,nrow = N.replicates, ncol = 4)
  moments_em <- matrix(NA, nrow =N.replicates, ncol =6)
